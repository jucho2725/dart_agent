...
def analyze_financial_statements(df, verbose=True):
    """재무제표 데이터를 분석하는 함수"""
    ...
    # 주요 계정 분석 (당기금액 기준)
    if 'thstrm_amount' in df.columns and verbose:
        # 금액이 있는 데이터만 필터링
...
def print_dataframe_info(df, data, verbose=True):
    ...
    if verbose:
        ...
        # 재무제표 분석
        analyze_financial_statements(df, verbose=verbose)
...
def save_as_json(df, filename, verbose=True):
...
def get_financial_statement_for_company(api_key, corp_code, company_name, output_format="dataframe", 
                                        bsns_year="2023", reprt_code="11011", verbose=True):
    ...
    if output_format == "json":
        ...
        for fs_name, (df, raw_data) in results.items():
            ...
            try:
                ...
                if not os.path.exists(json_filename):
                    save_as_json(df, json_filename, verbose=verbose)
                else:
                    if verbose:
                        print(f"⏭️  {fs_name} 파일 '{json_filename}'이 이미 존재합니다. 건너뜁니다.")
            except Exception as e:
                if verbose:
                    print(f"JSON 파일 저장 중 오류 발생: {e}")
    else:
        ...
        for fs_name, (df, raw_data) in results.items():
            if verbose:
                print(f"\n{'='*20} {fs_name} {'='*20}")
                print_dataframe_info(df, raw_data, verbose=verbose)
            ...
            try:
                ...
                if not os.path.exists(csv_filename):
                    ...
                    print(f"\n⏭️  {fs_name} 파일 '{csv_filename}'이 이미 존재합니다. 건너뜁니다.")
            except Exception as e:
                print(f"❌ 파일 저장 중 오류 발생: {e}")
    
    return results

def main(company_name=None, output_format="dataframe", bsns_year="2023", reprt_code="11011", verbose=True):
    """메인 실행 함수"""
    ...
    # 4. 재무제표 데이터 조회
    return get_financial_statement_for_company(api_key, corp_code, company_name, output_format, bsns_year, reprt_code, verbose=verbose)

def test_samsung(verbose=True):
    """삼성전자 테스트 함수"""
    if verbose:
    ...
    # 삼성전자 재무제표 데이터 조회
    return get_financial_statement_for_company(api_key, corp_code, "삼성전자", "dataframe", verbose=verbose)
